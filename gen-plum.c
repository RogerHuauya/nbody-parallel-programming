...
double m[N_MAX], x[N_MAX][3], v[N_MAX][3], mcm, xcm[3], vcm[3];

double X, Y, Z, Vx, Vy, Vz, X1, X2, X3, X4, X5, X6, X7, R, Ve, V,
    M = 1.0, eps = 0.0, EK = 0.0, EK_teor, conv, tmp;
...
double my_rand(void) { return ((double)(rand() / (double)RAND_MAX)); }
/***************************************************************/
...
int main(int argc, char *argv[]) {
  ...
  for (i = 0; i < N; i++) {
    fprintf(out,
            "%06d   %.16E   % .16E % .16E % .16E   % .16E % .16E % .16E \n", i,
